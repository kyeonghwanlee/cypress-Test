...
import { flattenObjectTree, renameInFlattenedTree, FileType, stringify, AppConfigError, FailedToSelectSubObject, EmptyStdinOrPromptResponse, } from '@app-config/core';
import { promptUser, consumeStdin } from '@app-config/node';
import { checkTTY, LogLevel, logger } from '@app-config/logging';
import { loadUnvalidatedConfig, loadValidatedConfig, } from '@app-config/config';
import { keyDirs, initializeLocalKeys, loadPrivateKeyLazy, loadPublicKeyLazy, encryptValue, decryptValue, loadKey, initializeKeys, deleteLocalKeys, loadSymmetricKeys, saveNewSymmetricKey, generateSymmetricKey, latestSymmetricKeyRevision, loadTeamMembersLazy, trustTeamMember, untrustTeamMember, shouldUseSecretAgent, startAgent, disconnectAgents, } from '@app-config/encryption';
...
async function loadConfigWithOptions({ secrets: includeSecrets, select, noSchema, fileNameBase, environmentOverride, environmentVariableName, }) {
    ...
    if (select) {
        // @ts-ignore
        jsonConfig = (await resolve.apply(RefParser, [jsonConfig])).get(select);
...
async function loadVarsWithOptions({ prefix, rename, alias, only, ...opts }) {
...
export const cli = yargs
    ...
    builder: (args) => args
        ...
        if (initialized === false) {
            throw new AppConfigError('Secrets were already initialized. Reset them if you want to create a new key.');
        }
    ...
    if (opts.prefix.length > 0) {
        ...
        if (schema && (!opts.only || opts.only.includes(schemaVariableName))) {
            env[schemaVariableName] = stringify(schema, fileTypeForFormatOption(opts.format), true);
        }
    }
