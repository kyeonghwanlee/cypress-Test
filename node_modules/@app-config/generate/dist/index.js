...
exports.generateQuicktype = exports.generateTypeFiles = void 0;
...
async function generateTypeFiles({ directory, schemaOptions, metaOptions } = {}) {
    var _a, _b;
    const { schema } = await (0, schema_1.loadSchema)(Object.assign({ directory }, schemaOptions));
    const { value: { generate = [] }, } = await (0, meta_1.loadMetaConfig)(Object.assign({ directory }, metaOptions));
    ...
    await Promise.all(generate.map(async ({ file, type = (0, path_1.extname)(file).slice(1), name = 'Config', augmentModule = true, leadingComments, rendererOptions = {}, }) => {
        logging_1.logger.info(`Generating ${file} as ${type}`);
        const lines = await generateQuicktype(schema, type, name, augmentModule, leadingComments, rendererOptions);
        await (0, fs_extra_1.outputFile)((0, path_1.join)(metaDirectory, file), `${lines.join('\n')}${'\n'}`);
    }));
    return generate;
}
exports.generateTypeFiles = generateTypeFiles;
async function generateQuicktype(schema, type, name, augmentModule = true, leadingCommentsOverride, rendererOptions = {}) {
    const src = {
        name,
        schema: JSON.stringify(schema),
    };
    const inputData = new quicktype_core_1.InputData();
    await inputData.addSource('schema', src, () => new quicktype_core_1.JSONSchemaInput(undefined));
    ...
    if (['ts', 'typescript', 'flow'].includes(type)) {
        Object.assign(rendererOptions, Object.assign({ 'just-types': 'true', 'runtime-typecheck': 'false' }, rendererOptions));
    }
    if (['rs', 'rust'].includes(type)) {
        leadingComments = [];
        Object.assign(rendererOptions, Object.assign({ 'derive-debug': 'true', 'edition-2018': 'true' }, rendererOptions));
    }
    ...
    if (['go', 'golang'].includes(type)) {
        ...
        const singleton = (0, common_tags_1.stripIndent) `
      var config ${name}
    ...
    `.split('\n');
        const loadConfig = (0, common_tags_1.stripIndent) `
...
exports.generateQuicktype = generateQuicktype;
