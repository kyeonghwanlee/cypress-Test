...
const TEST_FAILED =
  "##teamcity[testFailed name='%s' message='%s' details='%s' captureStandardOutput='true' flowId='%s']";
const TEST_FAILED_COMPARISON =
...
function Teamcity(runner, options = {}) {
  ...
  reporterOptions.flowId
    ? (flowId = reporterOptions.flowId)
    : (flowId = process.env["MOCHA_TEAMCITY_FLOWID"] || processPID);
  reporterOptions.useStdError
    ? (useStdError = reporterOptions.useStdError)
    : (useStdError = process.env["USE_STD_ERROR"]);
  reporterOptions.recordHookFailures
    ? (recordHookFailures = reporterOptions.recordHookFailures)
    : (recordHookFailures = process.env["RECORD_HOOK_FAILURES"]);
  reporterOptions.actualVsExpected
    ? (actualVsExpected = reporterOptions.actualVsExpected)
    : (actualVsExpected = process.env["ACTUAL_VS_EXPECTED"]);
  useStdError ? (useStdError = useStdError.toLowerCase() === "true") : (useStdError = false);
  recordHookFailures
    ? (recordHookFailures = recordHookFailures.toLowerCase() === "true")
    : (recordHookFailures = false);
  ...
  runner.on("fail", function (test, err) {
    if (actualVsExpected && err.actual && err.expected) {
      if (useStdError) {
        logError(
          formatString(TEST_FAILED_COMPARISON, test.title, err.message, err.stack, err.actual, err.expected, flowId)
        );
      } else {
